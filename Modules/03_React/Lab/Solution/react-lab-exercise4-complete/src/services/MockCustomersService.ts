...
export default class MockCustomersService implements ICustomerService {
    ...
    getCustomersByLastName = (lastNameSearch: string): Promise<ICustomer[]> => {
        ...
        let customers: ICustomer[] = this.customers.map(customer => <ICustomer>{
            "CustomerId": customer.CustomerId,
            "FirstName": customer.FirstName,
            "LastName": customer.LastName,
            "Company": customer.Company,
        ...
        let filteredCustomers = customers.filter(customer => customer.LastName.toLowerCase().startsWith(lastNameSearch.toLowerCase()))!;
